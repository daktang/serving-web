...
먼저 n \times m모양의 직사각형 모양 보드판에 숫자들을 나열한다.
...
n과 m중 더 작은 수는 무조건 짝수이다.(이는 맨 가운데 회로가 반드시 빈칸 없이 존재한다는 것을 의미한다.) (2 \le n,m \le 300, 1 \le r \le 10^9)
다음 n개 줄에는 m개의 정수가 주어지며, 이는 각 좌표별로 보드판에 써 있는 숫자들의 초기 상태이다. 
정수들은 10^9 이하이다. 입력 데이터의 50%에 있어서 1 \le r \le 1,000을 만족함을 보장한다.
